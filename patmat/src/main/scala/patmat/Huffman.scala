...
object Huffman {
  ...
  def chars(tree: CodeTree): List[Char] = tree match {
    case Fork(l, r, crs, w) => chars(l) ::: chars(r)
    case Leaf(c, w) => c :: Nil
  }
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    ...
    def iter(times: Int, n: Int, c: Char, pairs: List[(Char, Int)]): List[(Char, Int)] = {
      if (n == sortedChars.length) (sortedChars(n - 1), times + 1) :: pairs
      else if (sortedChars(n) != c) {
        iter(0, n + 1, sortedChars(n), (sortedChars(n - 1), times + 1) :: pairs)
      }
      else iter(times + 1, n + 1, c, pairs)
    }
    iter(-1, 0, sortedChars(0), Nil)
  }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    val frequencyTupleList = isortTuple(freqs)
    def iter(freqs: List[(Char, Int)], acc: List[Leaf]): List[Leaf] = {
      if (freqs == Nil) acc
      else iter(freqs.tail, (Leaf(freqs.head._1, freqs.head._2) :: acc))
    }
    iter(frequencyTupleList, Nil)
  }
  ...
  def isLeaf(tree: CodeTree): Boolean = tree match {
    case Fork(l, r, crs, w) => false
    case Leaf(c, w) => true
  }

  def singleton(trees: List[CodeTree]): Boolean = {
    if (trees.length == 1 && !isLeaf(trees.head)) true
    else false
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    def combineFirstTwo(trees: List[CodeTree]): List[CodeTree] = trees match {
  ...
  /****
    * go deeper into tree in L or R direction
    */
